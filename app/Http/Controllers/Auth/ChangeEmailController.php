  ...
  */
  ...
  public function reset_email(Request $request)
  {
    ...
    if (trim(strtolower($input['email1'])) == trim(strtolower($input['email2']))) {
      return redirect()->back()->withErrors(['not_equal'=>Lang::get('controller/changeEmail.not_equal')]);
    }
    ...
    if (!$user || $possible_user) {
      return redirect()->back()->withErrors(['is_taken'=>Lang::get('controller/changeEmail.is_taken')]);
    }
    ...
    return redirect(route('home'))->with('status', Lang::get('controller/changeEmail.success'));
